...
const CONSOLE_FIFO_RELATIVE_PATH = join('.cynexgp', 'console.in');
...
export function buildInitScript(originalEntrypoint: string[], originalCmd: string[]): string {
  ...
  const lines = [
    '#!/bin/sh',
    '',
    '# Patch hostname so kernel-level tools report "cynexgp"',
    "echo 'cynexgp' > /etc/hostname 2>/dev/null || true",
    'hostname cynexgp 2>/dev/null || true',
    '',
    '# Patch /etc/passwd so "whoami" and shell prompts show "cynexgp"',
    'if [ -f /etc/passwd ]; then',
    "  sed -i 's|^container:|cynexgp:|' /etc/passwd 2>/dev/null || true",
    "  sed -i 's|^user:|cynexgp:|'      /etc/passwd 2>/dev/null || true",
    "  sed -i 's|^app:|cynexgp:|'       /etc/passwd 2>/dev/null || true",
    'fi',
    '',
    ...
    '  if [ -f "$_rc" ]; then',
    "    sed -i 's/petrodactyl/cynexgp/g' \"$\\_rc\" 2>/dev/null || true",
    "    grep -q 'PS1.*cynexgp' \"$\\_rc\" 2>/dev/null || echo 'export PS1=\"container@cynexgp \\\\w \\\\\\$ \"' >> \"$\\_rc\"",
    '  fi',
    'done',
    '# Fish uses a different syntax for prompts',
    'if [ -f /home/container/.config/fish/config.fish ]; then',
    "  sed -i 's/petrodactyl/cynexgp/g' /home/container/.config/fish/config.fish 2>/dev/null || true",
    'fi',
    '',
    'export PS1="container@cynexgp \\w \\$ "',
    '',
    '# Set up the console FIFO (named pipe) for command input',
    'CYNEXXGP_CONSOLE_FIFO=/home/container/.cynexgp/console.in',
    'if [ ! -p "$CYNEXXGP_CONSOLE_FIFO" ]; then',
    '  rm -f "$CYNEXXGP_CONSOLE_FIFO"',
    '  mkfifo "$CYNEXXGP_CONSOLE_FIFO"',
    'fi',
    '',
    ...
    `while true; do cat "$CYNEXXGP_CONSOLE_FIFO"; done | ${startLine}`,
  ];
...
export async function startContainer(
  ...
  const cynexgpDir = join(volumePath, '.cynexgp');
  if (!existsSync(cynexgpDir)) mkdirSync(cynexgpDir, { recursive: true });

  const initScript = buildInitScript(originalEntrypoint, originalCmd);
  writeFileSync(join(cynexgpDir, 'init.sh'), initScript, {
    mode: 0o755,
    encoding: 'utf8',
  });

  modifiedEnv.PS1 = 'container@cynexgp \\w \\$ ';
  modifiedEnv.PROMPT = 'container@cynexgp \\w \\$ ';
  modifiedEnv.prompt = 'container@cynexgp \\w \\$ ';
  ...
  const container = await docker.createContainer({
    ...
    Entrypoint: ['/bin/sh', '/home/container/.cynexgp/init.sh'],
...
async function writeCommandToConsoleFifo(id: string, command: string): Promise<void> {
  ...
  const proc = Bun.spawn(['sh', '-c', 'printf "%s\\n" "$1" > "$2"', 'cynexgp-console-command', command, fifoPath], {
    stdout: 'ignore',
    stderr: 'pipe',
  });
