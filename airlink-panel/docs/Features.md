# CynexGP Panel - Feature Guide

This is a reference for everything in the CynexGP Panel. It covers the admin side, the user side, the addon system, and the API.
...
### Quick install
```bash
bash <(curl -s https://raw.githubusercontent.com/xAyan55/cynex/refs/heads/main/installer.sh)
```
...
Nodes are the machines that run game servers. Each node runs the CynexGP daemon, which the panel communicates with.
...
The marketplace tab lists all community addons from the `xAyan55/cynex-addons` registry. From here you can install addons with one click - the panel clones the repository, runs `pnpm install` and `pnpm run build`, and streams the output back to the browser.
