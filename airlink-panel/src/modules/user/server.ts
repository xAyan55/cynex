...
import { CynexGPCloudClient } from '../../handlers/utils/core/cynexgpCloud';
        ...
        try {
          ...
          if (response.data.success) {
            ...
            if (isCloudBackupEnabled) {
              try {
                const cloudClient = new CynexGPCloudClient(settings.airlinkCloudApiKey!);
                ...
                logger.error('Failed to redirect backup to CynexGP Cloud:', cloudError);
            ...
            res.json({
              success: true,
              message: isCloudBackupEnabled && airlinkCloudId ? 'Backup created and uploaded to CynexGP Cloud' : 'Backup created successfully',
              backup: {
                ...backup,
        ...
        try {
          ...
          if (backup.airlinkCloudId) {
            ...
            if (!settings?.airlinkCloudApiKey) {
              res.status(500).json({ error: 'CynexGP Cloud API key not configured' });
              return;
            }
            ...
            try {
              ...
              logger.error('Failed to prepare CynexGP Cloud backup for restore:', err);
      ...
      async (req: Request, res: Response) => {
        ...
        try {
          ...
          if (backup.airlinkCloudId) {
            ...
            if (settings?.airlinkCloudApiKey) {
              const cloudClient = new CynexGPCloudClient(settings.airlinkCloudApiKey);
              await cloudClient.deleteFile(backup.airlinkCloudId).catch(e => logger.warn(`Failed to delete backup from CynexGP Cloud: ${e}`));
            }
          } else {
            try {
              await axios.delete(
