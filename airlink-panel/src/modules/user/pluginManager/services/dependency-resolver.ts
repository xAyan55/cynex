...
export class DependencyResolver {
  ...
  private pickVersion(
    ...
    return versions.find((entry) => {
      const matchesGame = !minecraftVersion || entry.game_versions.includes(minecraftVersion);
      const matchesLoader = !loader || entry.loaders.some((value) => value.toLowerCase() === loader.toLowerCase());
